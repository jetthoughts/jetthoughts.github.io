...
class ArticleUpdater
  ...
  def call
    unsynced_articles.each do |article_id, attributes|
  ...
  def update_canonical_url_on_dev_to(article_id, slug)
    ...
    begin
      ...
      if response.is_a?(Net::HTTPSuccess)
        ...
        raise "Failed to update canonical_url: #{response.message} (Attempt #{attempt})"
      end

    rescue StandardError => e
      if attempt < max_retries
        puts "Error updating canonical_url for article #{article_id}, retrying in #{attempt * 2} seconds..."
        sleep(attempt * 2) # Задержка, увеличивающаяся с каждой неудачной попыткой
        retry
      else
