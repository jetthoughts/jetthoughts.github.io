...
class HugoPartialsTest < ApplicationSystemTestCase
  ...
  def test_header_partial_rendering
    ...
    within "header" do
      ...
      assert_selector "a.logo", 
        message: "Header should contain logo link"
      assert_selector "svg",
        message: "Logo should be SVG format"
      
      # Navigation partial
      assert_selector "nav.navigation",
        message: "Header should contain navigation"
      
      # Accessibility features
      assert_selector "a.skip-link",
        message: "Should have skip-to-content link for accessibility"
    end
  end
  ...
  def test_footer_partial_structure
    ...
    within "footer" do
      ...
      assert_selector ".info",
        message: "Footer should have company info section"
      
      # Contact information
      assert_selector "a[href*='mailto:']",
        message: "Footer should have email contact"
      assert_selector "a[href*='tel:']", 
        message: "Footer should have phone contact"
      
      # Navigation sections
      assert_selector "nav[aria-label*='Services']",
        message: "Footer should have services navigation"
      assert_selector "nav[aria-label*='Company']",
        message: "Footer should have company navigation"
      
      # Social media partial
      assert_selector "nav[aria-label*='Social']",
        message: "Footer should have social media links"
      
      # Copyright and legal
      assert_selector ".footer-bottom",
        message: "Footer should have bottom section"
      assert_text /© \d{4}/,
  ...
  def test_blog_post_partials
    ...
    if has_selector?(".blog-posts, .post-list, article")
      ...
      articles.first(3).each do |article| # Test first few articles
        within article do
  ...
  def test_favicon_partial
    ...
    within "head" do
      # Standard favicon
      assert_selector "link[rel*='icon']", minimum: 1,
        message: "Should have favicon links"
      
      # Apple touch icon
      assert_selector "link[rel='apple-touch-icon']",
        message: "Should have Apple touch icon"
      
      # Web manifest
      assert_selector "link[rel='manifest']",
        message: "Should have web app manifest"
      
      # Theme color
      assert_selector "meta[name='theme-color']",
        message: "Should have theme color for mobile browsers"
    end
  end
