...
- **Behavioral Testing Only**: Zero tolerance for structural/CSS/HTML testing
- **Visual Regression Required**: UI tests must use screenshot comparison

## 🚨 TDD Agent Behavioral Constraints (ZERO TOLERANCE)

### Prohibited Test Patterns - Agent Must Reject

**CRITICAL**: Agents MUST refuse to write or accept tests that validate implementation details instead of user-visible behavior.

#### **❌ CSS/HTML Structural Testing (BLOCKED)**

**Pattern Recognition**: Agent detects these anti-patterns and REFUSES to proceed:

```ruby
# ANTI-PATTERN 1: CSS class existence testing
page_html.scan(/class=".*fl-node-content.*"/).length >= 4  # BLOCKED
page.has_css?(".fl-module-content")                        # BLOCKED
page.find(".px-4.py-2")                                    # BLOCKED

# ANTI-PATTERN 2: HTML structure validation
assert_equal 3, page.all("div.container").count            # BLOCKED
hero_instances = page_html.scan(/class=/).length           # BLOCKED

# ANTI-PATTERN 3: Framework-generated selector usage
page.find(".fl-node-content")                              # BLOCKED
page.has_css?(".text-blue-500")                            # BLOCKED
page.find("[data-fl-node-id]")                             # BLOCKED

# ANTI-PATTERN 4: Method existence testing
assert SomeClass.method_defined?(:some_method)             # BLOCKED
```

**Agent Response When Detecting These Patterns**:
```
🚫 BLOCKED: Structural test pattern detected

Pattern: page_html.scan(/class="[^"]*fl-node-content/)
Issue: Testing CSS class existence (implementation detail)

❌ This test validates STRUCTURE, not BEHAVIOR
✅ Required: Test what users see/interact with

CORRECT ALTERNATIVES:
1. Visual regression: screenshot_path = take_screenshot("hero")
                      assert_visual_match(screenshot_path, baseline, tolerance: 0.03)

2. Content testing: assert page.has_content?("Fractional CTO Services")
                     assert page.has_css?("h1", text: "Fractional CTO")

3. Interaction test: click_button "Get Started"
                      assert_current_path "/contact"

I cannot proceed with structural tests. Please specify behavioral requirements.
```

#### **✅ Required Behavioral Testing Patterns**

**Pattern Recognition**: Agent enforces these correct patterns:
...
# ✅ PATTERN 2: User-visible content testing
def test_hero_section_displays_key_content
  visit "/services/fractional-cto/"
  assert page.has_content?("Fractional CTO Services")
  assert page.has_css?("h1", text: "Fractional CTO")  # Semantic element with text
  assert page.has_css?("button", text: "Get Started")  # Interactive element with label
end

# ✅ PATTERN 3: User interaction testing
def test_cta_button_navigates_to_contact
  visit "/services/fractional-cto/"
...
# ✅ PATTERN 4: Semantic HTML structure testing
def test_page_has_proper_heading_structure
  visit "/services/fractional-cto/"
  assert page.has_css?("h1", count: 1)     # Semantic element
  assert page.has_css?("main")             # Landmark element
  assert page.has_css?("nav")              # Navigation element
end
```

#### **🛡️ Agent Pre-Implementation Validation**

**MANDATORY**: Before writing any test, agent MUST validate:

```yaml
test_validation_checklist:
  question_1: "Does this test validate what users see or interact with?"
    - ✅ YES → Proceed with behavioral test
    - ❌ NO → Reject, request behavioral requirement

  question_2: "Does this test use CSS class names or HTML structure?"
    - ✅ YES → BLOCKED, request visual regression or semantic test
    - ❌ NO → Proceed with validation

  question_3: "Does this test use framework-generated selectors (FL-Builder, Tailwind)?"
    - ✅ YES → BLOCKED, request semantic or content-based test
    - ❌ NO → Proceed with validation

  question_4: "Could this test break if CSS refactoring changes class names?"
    - ✅ YES → BLOCKED, request behavior-focused test
    - ❌ NO → Proceed with implementation

  question_5: "Does this test use semantic HTML elements (h1, button, main)?"
    - ✅ YES → Approved for implementation
    - ❌ NO → Review approach, consider semantic selectors
```

#### **📋 Agent Test Smell Detection Protocol**

**AUTOMATED DETECTION**: Agent identifies test smells before implementation:

**Detection Pattern 1: CSS Class Testing**
```ruby
# Trigger: \.scan\(.*class=.*\)\.length
# Trigger: has_css\?\(["']\.[\w-]+["']\)
# Trigger: page_html\.scan\(.*class.*\)

# Agent Response:
"🚫 CSS class testing detected. This is a structural test (implementation detail).
Use visual regression (screenshot comparison) or content-based assertions instead."
```

**Detection Pattern 2: HTML Structure Validation**
```ruby
# Trigger: assert.*\.all\(.*\)\.count
# Trigger: assert.*method_defined\?

# Agent Response:
"🚫 HTML structure counting detected. This tests structure, not behavior.
Use semantic element assertions (h1, button, main) or visual regression instead."
```

**Detection Pattern 3: Framework Selector Usage**
```ruby
# Trigger: fl-node-content|fl-module-content|fl-builder
# Trigger: \.\w+-\d+  # Tailwind utility classes
# Trigger: data-fl-.*|data-node-

# Agent Response:
"🚫 Framework-generated selector detected. These are implementation details that change.
Use semantic HTML elements, content assertions, or visual regression instead."
```

#### **🎯 Agent Success Criteria**

**Test Acceptance Requirements**:
- ✅ Tests validate user-visible behavior (content, interactions, visual appearance)
- ✅ Tests use semantic HTML elements (h1, button, main, nav) not CSS classes
- ✅ Visual UI tests use screenshot comparison with baseline
- ✅ Tests survive CSS refactoring (class name changes don't break tests)
- ✅ Tests document expected user experience, not implementation details

**Test Rejection Requirements**:
- ❌ Tests validating CSS class existence or counts
- ❌ Tests counting HTML elements or structure validation
- ❌ Tests using framework-generated selectors (FL-Builder, Tailwind)
- ❌ Tests checking method existence without behavior validation
- ❌ Tests that break when CSS/HTML refactoring changes class names
...
- **Retrospective Framework**: See `docs/20-29-testing-qa/25.01-retrospective-framework-reference.md`
- **Failure Pattern Recognition**: See `docs/20-29-testing-qa/25.02-failure-pattern-library-reference.md`
- **Retrospective Recording**: See `docs/20-29-testing-qa/25.03-retrospective-recording-how-to.md`
- **Agile Development**: See `docs/60-69-project-management/60.02-agile-sprint-framework.md`
...
1. **REFERENCE** the SEO checklist in `docs/workflows/blog-pipeline.md` (the old `docs/seo/80.01` file never existed; docs/seo/ was archived 2026-08-08)
...
## 🔍 Retrospective Integration (MANDATORY)
...
pre_task_retrospective_check() {
    echo "🔍 Checking retrospective knowledge for task: $TASK"
...
post_task_retrospective_contribution() {
    echo "📚 Contributing learning outcomes to retrospective memory"
