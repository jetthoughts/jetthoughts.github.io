...
- **📦 Input:**
  ...
  | 2-3 `[verbatim customer quote]` lines | Run Perplexity prompt in Step 1: searches Reddit/G2 for real complaints using `[CUSTOMER]` and `[PROBLEM]` from 1.1. Keep 2-3 best quotes AND their source URLs (needed for 2.3 people-hunting). | **1.1 blanks** → Perplexity search |
...
| Channel | Budget (300 visits) | CPC range | Best for |
|---|---|---|---|
| Meta | $250-$700 | $0.70-$1.90 | B2C, broad consumer |
| Reddit | $375-$900 | $1.25-$3 | Niche communities, B2B |
| Google Search | $900-$2,400 | $3-$8 | Existing search demand |
| LinkedIn | $1,650-$6,600 | $5.70-$22 | B2B, named companies |
| Organic (free) | $0 | $0 | Slow but real — social sharing |
...
- **📝 Template — 5 interview questions from the Mom Test table:**
  ...
  | 2-3 ICP-specific probes | Derived from the 1.1 `[problem]` and `[competition]` blanks — ask about specific workarounds | **1.1 blanks** |
...
- **📦 Input:**
  | Input | How Sam obtains it | Upstream source |
  |---|---|---|
  | BUILD decision | From 2.5 synthesis | **2.5** |
  | Validated Problem Statement | `Validated Problem Statement` Google Doc | **2.5** |
  | 5 strongest-signal interviewees (≥7) | Names from 2.5 scoring sheet | **2.5** |
- **📝 3-screen prototype prompt:**
  | Blank | How Sam fills it | Source |
  |---|---|---|
  | `[PRODUCT CATEGORY]` | Derived from the problem statement — what kind of thing is this? | **2.5 problem statement** |
  | `[customer]` | The persona from the validated problem statement | **2.5** |
  | `[PRIMARY ACTION]` | The one thing the user does — verb-led, from the problem statement | **2.5** |
  | `[FAKE DATA]` | Realistic placeholder data Sam makes up matching the persona's world | Invented — realistic to the persona |
  | `[BUTTON LABEL]` | The CTA verbatim from 2.5 interviewee vocabulary | **2.5 verbatim language** |
...
- **📦 Input:**
  | Input | How Sam obtains it | Upstream source |
  |---|---|---|
  | Validated Problem Statement | Open `Validated Problem Statement` Google Doc in `Founder OS` folder | **2.5** |
  | Verbatim prototype vocabulary | Notes from 2.6 closing question — the words interviewees used to describe the product | **2.6** |
- **📝 Vibe PRD Template — 5 sections:**
  | Section | How Sam fills it | Source |
  |---|---|---|
  | **1. The problem** | Copied **word-for-word** from 2.5 validated problem statement. Do not paraphrase. | **2.5 (verbatim copy)** |
  ...
  | **3. What you're building** | One verb-led paragraph. Input → output. Uses 2.6 prototype observations — what the 5 interviewees actually tried to do. | **2.6 observations** |
...
- **📝 Build Path Decision Worksheet — 5 yes/no questions:**
  ...
  |---|---|---|
  | Can I describe the build in one sentence? | Yes if brief Section 3 fits one paragraph | **3.2** |
  | Is my runway >4 months? | Sam's bank account / runway calculation | Sam knows this |
  | ... (3 more questions from worksheet) | ... | **4.1 lesson worksheet** |
...
- **🔑 Setup:**
  ...
  | GitHub sync | In Lovable Settings → turn on GitHub sync (pre-flight rule 3). Uses GitHub account from 4.2. | **4.2 GitHub account** |
- **📦 Input:**
  | Input | How Sam obtains it | Upstream source |
  |---|---|---|
  | "Self-serve" build decision | From 4.1 worksheet | **4.1** |
  | Quality-checked one-page brief | `Product Brief` Google Doc — the 3.2 quality-checked version | **3.2** |
  | Passed ownership audit | 4.2 checklist completed | **4.2** |
- **📝 3 pre-flight rules:**
  | Rule | How Sam verifies it | Source |
  |---|---|---|
  | 1. Brief passed 3.2 quality-gate | Section 3 reads as outcomes, not features | **3.2 output** |
  | 2. One workflow, one persona, one happy path | Write one sentence naming all three — if it takes two sentences, scope is too big | **3.2 Section 3** |
  | 3. GitHub sync ON | Checked in Lovable Settings | **4.2 GitHub account** |
...
- **📦 Input:**
  | Input | How Sam obtains it | Upstream source |
  |---|---|---|
  | 3 pre-flight rules locked | Verified in 4.3 setup | **4.3** |
  | One-page brief from Module 3 | `Product Brief` Google Doc | **3.2** |
  | Scope-leak watchlist | 3 flagged features from Claude leakage check | **4.3** |
  | Supabase project | Created in 4.3 | **4.3** |
  | Lovable project with GitHub sync | Created in 4.3 | **4.3** |
  ...
  | Domain | Purchased in 4.2, now pointed to staging URL | **4.2** |
...
- **📝 5 green lights gate:**
  ...
  |---|---|---|
  | 1. Stripe in live mode, real card cleared | Test charge with own card, refund immediately | **1.5 Stripe setup** |
  | 2. Custom domain wired | Domain from 4.2 points to staging URL | **4.2 domain** |
...
- **📦 Input:** ≥10-30 users on MVP. **How Sam obtains them:**
  ...
  |---|---|
  | 4.4 onramp waitlist | Emails collected from the 1.2/1.4 landing page waitlist — export from Mixo "Subscribers" tab |
...
- **📝 5-question survey:**
  ...
  |---|---|---|
  | Q1: "How would you feel if you could no longer use this product?" | Copy verbatim from 5.1 lesson (Very disappointed / Somewhat disappointed / Not disappointed) | **5.1 lesson** |
  ...
  | Q5: Segment question (role / use case) | Sam defines based on 2.5 personas | **2.5 persona** |
...
- **🆘 Plan B — no segment above 25%:** Product problem → back to 4.4, ship against most common Q3 answer.
...
- **📝 6-column spreadsheet — How Sam seeds it:**
  ...
  | 1.4 waitlist signups | Export from Mixo "Subscribers" tab (or whatever builder Sam used in 1.2) | **[1.2](/course/tech-for-non-technical-founders-2026/smoke-test-build-page/)/[1.4](/course/tech-for-non-technical-founders-2026/smoke-test-landing-page-7-day-demand-test/)** |
  | 2.3 30-name list | Open the spreadsheet from 2.3 — many are still warm | **2.3** |
...
- **📦 Input:**
  | Input | How Sam obtains it | Upstream source |
  |---|---|---|
  | 50-name list with 4 buckets | Google Sheet from 5.3 | **5.3** |
  | Must-have segment name | From 5.1 survey results | **5.1** |
- **📝 4-part message structure:**
  | Part | How Sam fills it | Source |
  |---|---|---|
  | 1. Bucket-specific opener | Champions: "Since we last spoke..." / Hot: "You signed up for the waitlist..." / Warm: "I noticed you're in [industry]..." / Cold: "I saw your post about [topic]..." | **5.3 bucket labels** |
  | 2. One line on the problem in their language | Paste exact Q3 verbatim answers from 5.1 survey | **5.1 Q3** |
  | 3. 90-second Loom | Record: 60s product walkthrough (show the MVP from 4.4), 30s personal intro | **4.4 MVP** |
...
- **🔑 Setup:**
  ...
  |---|---|---|
  | Stripe | Already set up from 1.5. Creates a Payment Link in Dashboard for the deposit. | **1.5 Stripe account** |
...
- **📝 DPA Template — 6 sections + signature block. For every `[BRACKET]`:**
  ...
  | `[Customer Company Name]` | The company name of the warm lead from 5.5 tracking sheet | **5.5 lead info** |
  ...
  | `[Use case 1]`, `[Use case 2]` | 2 specific ways the customer will use the product. Named during the 5.5 demo. | **5.5 demo conversation** |
  | `[time]` `[timezone]` | Friday demo time agreed during 5.5 | **5.5 scheduling** |
  ...
  | `[Measurable criterion 1/2/3]` | Same as outcomes above but with a verifiable metric. "Report prep time reduced to ≤30 min/week, verified in Friday demo." | **2.5 problem cost** |
  ...
  | `[Champion Name]` | The person Sam demoed to in 5.5 — their name from the tracking sheet | **5.5 lead info** |
- **📝 Deposit math:** 1.5 price × 12 = year-one ACV. Deposit = 10-30%. **$500 floor.** When 10% of ACV < $300: charge first month upfront instead.
...
│
├─ 1.1: [customer] [problem] [approach] [competition] [differentiation] ← from Sam's head, refined by Perplexity in 1.2
│   │
│   ├─ 1.2: [CUSTOMER] [PROBLEM] [COMPETITION] [DIFFERENTIATION] ← COPY from 1.1 blanks
│   │       [verbatim quote 1/2/3] ← Perplexity search using 1.1 [customer] + [problem]
│   │       Hero image ← AI generator or Unsplash (free stock photos)
│   │
│   ├─ 1.3: no blanks — pastes 2 tracking snippets into 1.2 page
│   │
│   ├─ 1.4: campaign targeting ← 1.1 [customer] blank defines audience
│   │       ad account ← new signup, payment method from Sam's wallet
│   │
│   └─ 1.5: price number ← web search for 2-3 existing tools in category
│           price framing ← pick ONE from 1.5 lesson template
│           Stripe account ← bank + tax ID (SSN for US)
│
├─ 2.1: interview questions ← Mom Test table (2.1 lesson) + 1.1 [problem]/[competition] blanks
│   │
│   ├─ 2.2 (opt): same question list ← 2.1 draft, tested against AI personas
│   │
│   ├─ 2.3: ICP map prompt ← 1.1 [customer] + [problem] blanks + 1.2 Perplexity source URLs
│   │
│   ├─ 2.4: outreach messages ← 1.2 verbatim quotes + 1.1 [customer]/[problem] blanks
│   │
│   ├─ 2.5: Validated Problem Statement ← 10 interview transcripts (NOT 1.1 hypothesis)
│   │       Gate: ≥7/10 = BUILD, 4-6 = PIVOT, <4 = KILL
│   │
│   └─ 2.6: [PRODUCT CATEGORY] ← 2.5 problem statement
│           [customer] ← 2.5 validated persona
│           [PRIMARY ACTION] ← 2.5 problem statement
│           [FAKE DATA] ← invented, realistic to persona
│           [BUTTON LABEL] ← 2.5 verbatim interviewee language
│
├─ 3.1: Section 1 ← VERBATIM COPY of 2.5 problem statement
│       Section 2 ← 2.6 prototype vocabulary + 1.1 customer blank
│       Section 3 ← 2.6 prototype observations
│       Section 4 ← Sam defines from 2.5 problem cost
│       Section 5 ← Sam's judgment + AI leakage check
│   │
│   └─ 3.2: Section 3 rewrite ← 3.1 Section 3 (feature→outcome job stories)
│           Gate: reviewer names 0 things outside no-go list
│
├─ 4.1: build-path decision ← 3.2 quality-checked brief + Sam's runway
│   │
│   ├─ 4.2: GitHub ← new signup [github.com] with Gmail
│   │       AWS ← new signup [aws.amazon.com] with Gmail
│   │       Domain ← purchase at [porkbun.com] ~$10/yr
│   │
│   ├─ 4.3: Supabase ← new signup [supabase.com] with Gmail
│   │       Lovable ← signup [lovable.dev] (may exist from 2.6)
│   │       3 pre-flight rules ← 3.2 brief + scope sentence + GitHub sync
│   │       Scope-leak watchlist ← Claude leakage check on 3.2 Section 3
│   │
│   └─ 4.4: 4 build phases ← 3.2 brief + 4.3 stack + 4.2 domain
│           Stripe live mode ← 1.5 account, now switched to live
│           5 green lights ← each verifiable independently
│
└─ 5.1: 5 questions ← copy from 5.1 lesson + 2.5 persona for Q5
    │    Users ← 1.2/1.4 waitlist + 2.3-2.4 interviewees + 4.4 testers
    │
    ├─ 5.2 (opt): channel score ← 5.1 Q2-Q3 verbatim answers
    │
    ├─ 5.3: 50-name spreadsheet ← seed from 2.4 interviewees + 1.4 waitlist + 2.3 list + personal network
    │
    ├─ 5.4: Part 2 (problem language) ← 5.1 Q3 verbatim answers
    │       Loom recording ← 4.4 MVP walkthrough
    │
    ├─ 5.5: send sequence ← 5.3 bucket labels determine order
    │
    └─ 5.6: [Your Company Name] ← Sam knows this
            [Customer Company Name] ← 5.5 lead info
            [Outcome 1/2/3] ← 2.5 problem + 5.5 demo conversation
            [Use case 1/2] ← 5.5 demo conversation
            $[deposit] ← 1.5 price × 12 × 10-30%, $500 floor
            [Measurable criterion 1/2/3] ← 2.5 problem cost, verified in demo
            $[amount]/[month or year] ← 1.5 price, refined by M2-M5
            [Your Name] ← Sam knows this
            [Champion Name] ← 5.5 lead info
```
