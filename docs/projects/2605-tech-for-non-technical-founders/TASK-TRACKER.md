...
| 🔄 P1 | Course-wide voice cleanup sweep | M1 done 2026-06-12 (1.1, 1.2c, 1.3 polished cold-eyes); M2-M5 + walkthroughs pending | M1 cold-eyes sweep applied: Hook ≤3 sentences, em-dash → hyphen, error blocks normalized to `If this fails: / Why: / Fix:` triple, closure migrated to 4-slot pattern (`Done` / `You have now` / `Next` / `If blocked`), Outcomes re-cast to result-state. M1 budget/tool callouts already comparison-context-only. **Still queued for M2-M5 + walkthroughs:** (a) strip scattered budget mentions; (b) consolidate 3+ interchangeable tool listings; (c) rename 1.2b title "Wire Tracking Before You Spend a Dollar" → "...Before Traffic Starts" (13+ file blast radius across data/course_sequence.yaml, lessons referencing it, GOAL-AT-A-GLANCE, walkthrough); (d) closure 4-slot migration; (e) Output/Outcome Sam-voice rewrites. Gate: Phase 2 M2-M5 lesson creation starts AFTER this sweep completes. |
