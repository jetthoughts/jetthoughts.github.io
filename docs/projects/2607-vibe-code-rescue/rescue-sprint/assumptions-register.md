...
| E3 | **Delivery via BOTH routes - JT's own senior Rails contractors and a white-label partner - with Paul as trust layer (G0)** (revised by CEO 2026-08-09; previously white-label only) | in-house vs white-label → **both**: three unallocated senior Rails contractors at zero bench cost carry the first rescues, the partner carries capacity beyond them, fallback partner still to be named | partner route still validated by a PAID TRIAL rescue before selling (G0 gate); if that trial misses the quality bar, route the work in-house or to the fallback - delivery is no longer single-threaded, so a partner miss no longer stops the offer |
