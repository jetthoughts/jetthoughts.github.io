...
**Who does what.** The rescue is built under the JetThoughts brand, to our quality bar, through two routes (CEO 2026-08-09): JT's own senior Rails engineers and a white-label partner. Agents do all the prep - research, sourcing, drafting, the target list, the openers, the scripts. **Paul is reserved for the calls and the close.** A runbook plus an incremental task backlog holds the state so any session can pick up and execute without losing context.

## Rabbit holes

- **Delivery has to be guaranteed, not assumed.** The offer is only sellable if delivery holds up under our brand. As of 2026-08-09 this is no longer single-threaded - in-house engineers carry the first rescues and the partner carries capacity beyond them - but the partner leg is still un-validated: run the paid trial rescue and name a fallback before leaning on it. Do not sell partner capacity on a handshake.
