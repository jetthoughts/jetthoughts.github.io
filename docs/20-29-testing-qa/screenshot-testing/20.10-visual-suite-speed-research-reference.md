...
- **O5 — safe parallel bin/test + bin/dtest (added Paul 2026-08-01).**
  **(a) DONE 2026-08-01** — restore-on-green is now OS-scoped (bin/test
  scopes by uname, bin/dtest to linux/, bin/qtest by host_os); verified via
  smoke run + zero-dirty check. Scope (b) below remains open. Two
  scopes, originally both unsafe:
...
- **O6 — parallel `bin/dev` servers across worktrees (added Paul 2026-08-01).**
  ...
  must carry the printed port. Non-negotiable (Paul 2026-08-01): `bin/dev`
  is the ONLY sanctioned way for agents to serve the site — an agent hitting
  address-in-use must rely on bin/dev's port handling, never compose its own
  `hugo server` CLI line (custom lines skip the build pipeline flags and
  produce wrong-config servers). Acceptance: `bin/dev` in two worktrees
  simultaneously → both serve, distinct ports, each renders its own branch's
  content.
  (c) **worktree dtest is silently VACUOUS (found 2026-08-01, live):** a git
  worktree's `.git` is a pointer FILE to the main repo's git dir, which is
  NOT inside the container's `..:/app` mount — so git fails inside the
  container, snap_diff cannot check out git-HEAD baselines, every screenshot
  registers as "new" and passes silently (fail_if_new only hard-errors under
  CI env). Evidence: a worktree dtest run came back 34/34 green with NO
  [snap_diff] summary line and NONE of the 7 always-firing emulation diffs.
  Until fixed, bin/dtest MUST run from the main checkout only; candidate
  fixes: mount the worktree's gitdir into the container, or make bin/dtest
  refuse to run when `.git` is a file (guard + clear message).

- **O7b — SVG legibility-floor check — ✅ DONE 2026-08-01 (M2-M5 rollout 20%-slot).** `bin/check-svg-floor` flags course SVGs whose smallest text renders <9px@390 (viewBox-W vs min-font). Report-only (SVG_FLOOR_BLOCK=1 to make it blocking); flips to a build gate once the deferred ~46 convert. Confirmed: all 17 rollout + 6 M1 SVGs clear; 46 reference/walkthrough SVGs remain (the deferred-wave burn-down).
- **O7 — banned-strings validator net gap (found W2-T7, 2026-08-01).**
  `check_banned_strings` (lib/course_validators.rb:369) scans only pages with
  `course_chapter: true` - walkthroughs (no flag), `smoke-test-build-page`
  (explicit false), and `reference/stripe-price-test-full` sit outside the
  net; W2-T7 found 3 LIVE violations of signatures already in the yaml that
  green builds never caught. Fix: widen the scan to all pages under
  `content/course/<course>/` (or an explicit opt-out list), + a unit test
  proving a violation on a non-chapter page fails the build. lib/ change →
  needs `bundle exec rake test:unit` (CI runs it; test:critical does not).

- **O8 — worktree node_modules auto-setup — ✅ DONE 2026-08-01 (W3 20%-slot).** bin/hugo-build now auto-runs `bun install --frozen-lockfile` when node_modules is missing. Original note:
  Fresh git worktrees have no `node_modules`, so `bin/hugo-build` dies on
  "postcss: command not found" and EVERY parallel worktree agent must run
  `bun install` (or link the main repo's node_modules onto PATH) before the
  first build - friction hit by all 5 W2 workers + all 3 W3 workers. Fix:
  `bin/hugo-build` (or a `bin/worktree-setup` helper) detects a missing
  `node_modules` and runs `bun install --frozen-lockfile` (or symlinks the
  main checkout's) with a one-line notice, so agents don't each rediscover
  it. Cheap, removes friction from every future fan-out. Stronger 20%-slot
  candidate than O5(b) because it is the actual observed pain.
