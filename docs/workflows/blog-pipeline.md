...
- Length: as long as needed to deliver the solution to a real reader, no longer.
  Word count is not a target - soft, hard, or otherwise. Do not pad up to a
  minimum and do not trim down to a ceiling. The only judgment tool is the cut
  test below.
  - Cut test (apply to every H2, every paragraph, every sentence): ask "what
    does the reader DO differently after reading this?" If the answer is
    "nothing they didn't already know" or "skim and move on", DELETE it.
    Don't relegate to "further reading" - delete. Real readers benefit from
    concision; Google does not punish it.
  - A sentence that restates the H2 above it, the table next to it, the prior
    paragraph, or the lead is filler - cut. The reader already saw it.
