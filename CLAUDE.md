...
- **Feature-branch + PR for HTML/CSS/template changes (BLOCKING)**: Don't push HTML/CSS/template/layout changes directly to `master`. Always: (1) `git checkout -b <descriptive-branch>`, (2) commit on the branch with both test gates green, (3) `git push -u origin <branch>`, (4) open PR via `gh pr create` with summary + visual evidence (screenshot paths). This gives the user a review surface before changes hit the deployed site. Direct-to-master is only acceptable for content-only blog edits (markdown body without template changes), commit-message-only fixes, and docs under `docs/`.
