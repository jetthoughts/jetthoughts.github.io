...
- LinkedIn pipeline: `docs/workflows/linkedin-post-pipeline.md` (Paul Keen voice rules, AI score rubric, save-location convention). **Every LI post you CREATE or UPDATE ends its handback with the preview link** - `http://localhost:<your session port>/linkedin/<lane>/<slug>/` (dev-only mount), board at `/linkedin/`. Not just when Paul asks to "review": a reframe or a one-line edit ships with the link too. Never 1313 (see the session-random-port rule below), never paste the post body instead. Details: `linkedin-posts/README.md` §Review board
...
- **Zero ad hoc test scripts**: only the header **Test** line commands — never `*.sh` one-offs
- **No Python scripts for analysis**: Prefer `rg`, `sed`, `awk`, and shell tools
- **Never leave the shell in a subdirectory (2026-08-19)**: the Bash working directory PERSISTS between calls, so a bare `cd content` or `cd linkedin-posts/<lane>/assets` silently breaks every later repo-relative path - `.okf/`, `docs/`, `content/` all resolve against the subdir and report "No such file or directory", which reads as a missing file rather than a wrong cwd. Bit this session twice (a `content/_index.md` grep and a `.okf/design/house-visual-spec.md` read, both of which exist). Use absolute paths, or `cd /Users/.../jetthoughts.github.io && <cmd>` in the same call, or pass the directory to the tool (`rsvg-convert -o path/out.png path/in.svg`, `git -C`). If a file you know exists reports missing, run `pwd` before believing it.
...
- **Always end page-change handbacks with local review links (Paul 2026-08-17; scope widened 2026-08-19)**: after ANY change to something that renders locally, the handback message ends with the direct `http://localhost:<port>/<path>/` link(s) to every edited page (note hard-refresh if the page was open). Paul re-reviews in one click - never make him reconstruct the URL. **"Renders locally" is the test, not "lives in `content/`"** - it covers templates and CSS, and equally `linkedin-posts/` drafts, which render at `/linkedin/<lane>/<slug>/` via a dev-only mount. The narrower "rendered page (content, template, CSS)" wording let two LI rewrites ship linkless on 2026-08-19 because the drafts sit outside `content/`. If a local URL exists for the thing you changed, it goes in the handback.
...
- **Feature-branch + PR for ALL changes (BLOCKING - Paul 2026-08-17: "use PR for our changes, do not commit to master")**: Don't push directly to `master` - any change type, content-only edits and docs included. The pattern is: (1) `git checkout -b <sprint-name>`, (2) ship multiple related commits on the branch (one per fix is fine — easy to revert/cherry-pick), (3) run BOTH test gates green on each commit, (4) `git push -u origin <branch>`, (5) open ONE PR via `gh pr create` covering the full sprint with summary + per-commit description + visual evidence. **Bundle related work into one PR — don't split into many small PRs.** User flagged 2026-04-30: "let's have one big PR instead of small PR." A 5-commit sprint = 1 PR, not 5. **Cap: ~500 changed lines of CODE per PR (Paul 2026-08-21)** — code = templates, CSS, Ruby, scripts, config, content markup; **docs, `.okf/` logs, and binaries/baselines do NOT count**, so docs-only work may batch into bigger PRs. A sprint whose code diff would exceed the cap splits into sequential PRs, merge N before opening N+1. **Sprint/wave work ALWAYS rides its sprint's PR (Paul 2026-08-01: "use PR for our current sprints")** — including the sprint's docs, board updates, and review artifacts, which ride the same branch as the code they describe. Direct-to-master is DISABLED even for non-sprint changes (content-only edits, standalone docs, `CLAUDE.md` policy updates): branch + PR, always. Content-only PRs still run only the content gates (hugo-build + rendered scroll gate), per the content-only rule above. **`gh pr merge --auto` does NOT queue on this repo** — auto-merge is disabled, so it merges IMMEDIATELY (bit us 2026-08-14: used expecting it to hold for green, merged with a check still pending). To wait for green, poll `gh pr checks <n>` and merge without `--auto`.
  **When master moves under an open PR, REBASE - never `git merge origin/master` into the branch (Paul 2026-08-20: "instead of merging master we use rebase").** Three PRs in one day picked up merge commits, and each one dragged master's unrelated changes into the PR's own diff - #500 showed the entire blog rebuild until it was rebased, after which it showed 5 files. `git pull --rebase` / `git rebase origin/master`, then `git push --force-with-lease`. If unstaged changes block the update, `git pull --rebase --autostash origin master` — never a bare `git stash`. **Tag a backup ref first** (`git tag backup-<branch>`), per the branch-safety protocol - a rebase rewrites history.
  Trap when the branch was cut from ANOTHER feature branch that has since squash-merged: a plain rebase replays the already-merged commits (squashing changes their patch-id, so git cannot detect the duplicate) and they conflict with their own merged content. Don't fight it - `git reset --hard origin/master` and `git cherry-pick` only your own commits. Cut branches from `origin/master`, not from another branch, to avoid this entirely.
  Recurring cost worth knowing: `.okf/log.md` is one append-only file every concurrent session writes to, so it conflicts on essentially every parallel PR. Resolutions are near-always "keep both sides" - check before assuming a real conflict.
- **Agent 4-eyes on EVERY stage, author != verifier (Paul 2026-08-20 "re-review each changes before commit"; widened 2026-08-21)**: the point is agents collaborating to find the TRUTH — a single agent cannot find its own blind spot, because it checks the thing it MEANT to build. So: every change is **written by one sub-agent and verified by a different one**; the author never produces the evidence for its own claim, and the session's own self-review never counts as the second pair of eyes. Review gates every stage, not just the last — a wrong premise is load-bearing under everything built on it by the time a pre-merge review sees it. **The review lands before the artifact leaves the workshop**: a plan is peer-reviewed BEFORE the user is asked (they never see an unreviewed plan); a diff is peer-reviewed BEFORE commit (not "commit then review the PR"); a finding is reproduced before it is reported; a measurement is re-derived before it is quoted. Brief reviewers with the goal and the artifact, never with your conclusions, and ask for measurements rather than verdicts — a critic who returns a count cannot be argued with. **Reviewer routing, because gating everything with the slow reviewer is how 4-eyes gets abandoned**: internal sub-agents (distinct lens per call) for every per-stage review; `/codex:review` ONCE at the final verify before merge, never in the inner loop. Full protocol, evidence ladder and stopping rules: `docs/workflows/autonomous-delivery-prompt.md`. (Same-day evidence 2026-08-20: pre-commit reviewers caught a CSS-breaking template defect, a fabricated quote on a live page, and 3 orphaned references — all after gates were already green. 2026-08-21: 11 findings across 3 rounds on the delivery prompt itself, including two wrong counts and an audit pointed at a pointer-README instead of the real skill directories.)
