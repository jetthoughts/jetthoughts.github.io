...
- **Always end page-change handbacks with local review links (Paul 2026-08-17)**: after ANY change to a rendered page (content, template, CSS), the handback message ends with the direct `http://localhost:1313/<path>/` link(s) to every edited page (start `bin/dev` if not running; note hard-refresh if the page was open). Paul re-reviews in one click - never make him reconstruct the URL.
