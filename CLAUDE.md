...
- **Agent 4-eyes before every commit (Paul 2026-08-20: "re-review each changes before commit")**: before committing any non-trivial change, dispatch at least one reviewer agent on the actual diff — a correctness reviewer for code/template changes, plus a cold-eyes ICP reader for customer-facing copy. Blocking findings are fixed before the commit, and the verdict is quoted in the commit/PR. The session's own self-review does not count as the second pair of eyes. (Same-day evidence: pre-commit reviewers caught a CSS-breaking template defect, a fabricated quote on a live page, and 3 orphaned references — all after gates were already green.)
