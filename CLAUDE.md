...
- **Slop + shape-tell critic** — blog posts gate on `slop >= 8/10` (blog-pipeline.md STEP 5b; the `Slop ≤25` 0-100 scale is the COURSE gate — different scale, different direction, never mix them). Score is necessary, NOT sufficient; run the shape-tell critic after it passes; every revision runs the regression sweep before handback: `grep -rn "## Why this matters\|Founders who\|Most founders\|Founders we worked with"` across edited files + opener-shape count (no 3+ consecutive sections share a template). Details: voice-guide + memories.
