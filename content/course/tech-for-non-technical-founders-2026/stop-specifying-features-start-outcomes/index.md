...
- **I want [action + budget]** - a verb on a concrete object, with a limit. *"I want to grab the top 5 metrics in 30 seconds"* is a verb (grab), an object (the 5 metrics), and a budget (30 seconds). No 12-metric dashboard, no async export queue (background machinery that builds the file and emails it later).
