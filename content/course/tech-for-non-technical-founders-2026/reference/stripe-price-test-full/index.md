...
- **Live mode, not test mode.** A test-mode click only confirms the button is wired up - you need live mode to learn whether anyone actually wants to buy.
...
|---|---|---|
| Iterate before you kill | Under 5% | Lower price by 50%, add "founding member" framing, change button copy. Below 2% after that = audience or hypothesis is wrong, not the price. In the 2-4% band, also try shortening the Stripe product description. |
| Advance | 5-10% | Price-validated hypothesis. Move to Module 2 customer interviews. Email everyone who clicked but didn't complete - the most valuable interview group. |
| Verify before celebrating | Over 10% | Either exceptional fit or ad targeting hit a hot audience. Run a second cold channel briefly to verify. If both channels stay near the rate, the signal is real. If one is 12% and the other 2%, your targeting is unusually good, not your pricing. |
...
Two framings we keep reaching for on pre-product pages, instead of a plain "Get access - $97":
