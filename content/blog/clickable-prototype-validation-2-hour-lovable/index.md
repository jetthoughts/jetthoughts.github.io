...
metatags:
  ...
  og_description: "A 2-hour throwaway Lovable prototype lets 5 interview subjects experience the SHAPE of your solution. The third validation pillar after the landing page."
...
The prototype session is the third validation pillar. The [smoke test](/blog/smoke-test-landing-page-300-dollar-validation/) proves demand (strangers click). The Mom Test proves the problem is felt (specific past stories). The prototype session proves solution legibility (a user can work through it without coaching).
...
This prototype is throwaway. You discard it after validation. No production polish, no real auth, no payment integration, no real database. You are building a question - "Does the user know what to do?" - not a product. Three screens, fake data hard-coded in, CTAs that navigate but do not save.

Founders who try to "polish the prototype into the MVP later" spend 3-5x longer on it, add features that invalidate the shape test, and carry every 2-hour compromise into production. The [Module 5 Lovable build](/blog/self-serve-mvp-stack-lovable-supabase-stripe-2026/) starts fresh with a proper Vibe PRD, real auth, and a real database. This prototype starts fresh with one goal: three screens, two hours, five sessions, then archive.

## Build 3 Screens in 2 Hours with Lovable

[Lovable](https://lovable.dev) is an AI app builder that generates a working web app from a prompt. Free trial; paid plans from $20/month. No coding required.

Three screens is the constraint - not five, not ten - because each extra screen doubles the build time without sharpening the validation signal.
...
That prompt typically produces a navigable 3-screen prototype in Lovable within 20-30 minutes in our experience - results vary with prompt detail and Lovable model availability that day. Spend the remaining 100 minutes on one thing: reading the fake data out loud and asking yourself "does this make sense to someone who has never heard my idea?" If you hesitate, change the wording.
...
- **0 or 1 pass:** The shape is wrong. The problem statement may be right but the solution concept needs a different starting point. Return to Chapter 3.3 before writing any brief. Shape mismatch usually means the problem signal you got from interviews was off - re-running the Mom Test with sharper questions catches what a prototype rebuild would miss.
...
The pass count from the gate above tells you the next move; the failure notes and the exact vocabulary from the "describe in one sentence" closing question tell you what to do with it.

On a 4-5 pass, your validation stack is complete: demand signal from the smoke test, problem signal from the Mom Test, shape signal from the prototype sessions. Write the [One-Page Product Brief](/blog/one-page-product-brief-vibe-prd/) using those exact "describe in one sentence" words in the problem statement and in the Module 5 Lovable prompt - they are worth more than any marketing copy you will write.

On a 0-1 pass, the Mom Test still says the problem is real. What the prototype is telling you is that the solution you imagined does not match how users think about solving it. Before restarting, read the "what did you expect to see" answers - that is the user's mental model. A large mismatch (users expected an email tool, you built a dashboard) sends you back to Chapter 3.3 before writing the brief. A smaller mismatch (different starting step) is a revise-and-re-test.

Catching a shape mismatch here costs you two hours. Catching it in Module 5, after you have started the real build, costs three weeks. The throwaway prototype buys you the cheaper version of that lesson.
...
1. Open Lovable and run the persona-setup prompt with your 3 ICP variants - aim for a navigable 3-screen prototype before lunch.
2. Book 5 silent-observation sessions for this week with interviewees who scored 7+ on the Mom Test. Send the prototype link 10 minutes before each call.
3. After all 5 sessions, tally the pass count and copy the exact "describe in one sentence" answers into a doc - those words go into the [One-Page Product Brief](/blog/one-page-product-brief-vibe-prd/) verbatim.
