...
The sequence matters more than any individual technique. Here is what we changed, in order.
...
[Ruby's official benchmark suite](https://speed.ruby-lang.org/) shows current YJIT releases running roughly 90% faster than the interpreter on synthetic x86-64 benchmarks. That number is misleading for Rails apps; our production numbers landed at a 28-32% improvement in mean response time across all endpoints. Still significant for effectively zero code changes.
...
ZJIT exists as an experimental alternative, but it shows higher latency variance under load on the apps we've tested. For production Rails work today, YJIT is the only option we'd ship.
...
This isn't worth doing in every situation.
...
Total engineering time: roughly 60 hours across two developers. Monthly hosting cost dropped $1,200, and the support tickets about page-load complaints stopped showing up in the founder's Slack the week after the cache work shipped.
...
- [Rails Performance Improvements: Rails Blog](https://rubyonrails.org/blog)
