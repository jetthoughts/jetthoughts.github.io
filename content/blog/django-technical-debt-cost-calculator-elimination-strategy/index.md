...
Most teams we audit are losing meaningful capacity to technical debt every quarter. [Sonar's 2024 research on the cost of technical debt](https://www.sonarsource.com/blog/new-research-from-sonar-on-cost-of-technical-debt/) puts the per-codebase tax at roughly 5,500 developer-hours per year per million lines of code. On a team of 8 engineers at $150K average loaded cost, even a fraction of that runs into mid-six-figure annual exposure.
...
The turnover line hits hardest. SHRM's industry estimates put replacement cost at 50-200% of salary depending on role seniority once you factor recruiting, onboarding, and the months before a new hire is productive. We've watched two senior Django engineers leave back-to-back on a single rescue project where the codebase had become unmovable, and replacements took most of a quarter to ship the first feature.
...
**Bug resolution:** Average bug took 14 hours to fix. On a clean codebase of similar size, we'd expect closer to 3 hours. At about 4 bugs per developer per month, that's roughly 44 extra hours per developer per month going to triage instead of features.
...
[Sentry's writeup of a Django N+1 fix](https://blog.sentry.io/finding-and-fixing-django-n-1-problems/) reports cutting page load by 91% and dropping query volume by 950 per request. The real savings on the engagements we run come from developers who stop spending 10+ hours a month troubleshooting "why is the app slow."
...
Allocate 15-20% of each sprint to debt reduction. Push that higher and the product team revolts; push it lower and the debt accumulates faster than the team can pay it down. [Deloitte's analysis of technical debt](https://www.deloitte.com/us/en/insights/topics/technology-management/technical-debt-impact.html) is consistent with what we see on rescue engagements: ongoing debt-reduction practice produces meaningfully better delivery outcomes than one-time cleanup sprints.
